...
def slots_manager(building=None, slot=None):
    if request.method == 'GET':
        ...
        try:
            conn.execute(
                "DELETE FROM slots WHERE building=? AND slot=?", (building, slot))
        except sqlite3.Error as e:
            data, status = {"error": str(e)}, 400
...
def status_manager(building=None, slot=None):
    if request.method == 'POST':
        ...
        try:
            conn.execute(
                "UPDATE slots SET status=? WHERE building=? AND slot=?", (1, building, slot))
        except sqlite3.Error as e:
            data, status = {"error": str(e)}, 400
        ...
        try:
            conn.execute(
                "UPDATE slots VALUES SET status=? WHERE building=? AND slot=?", (0, building, slot))
        except sqlite3.Error as e:
            data, status = {"error": str(e)}, 400
